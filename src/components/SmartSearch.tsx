...
export default function SmartSearch({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        console.error("Failed to load products for smart search:", err);
      }
    };
    fetchAll();
  }, []);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setIsSearching(true);
    try {
      const data = await apiService.smartSearch(input.trim(), allProducts);
      if (data && Array.isArray(data.results)) {
        const matchedProducts = data.results.map((r: any) => {
          const original = allProducts.find(p => p.id === r.id);
          return original ? { ...original, aiReason: r.reason, aiHighlight: r.highlight } : null;
        }).filter(Boolean);
