...
export default function AISupportHub() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      if (!ai) {
        throw new Error("Connection failed: AI system offline (API Key required).");
      }
      const chat = ai.chats.create({
        ...
        config: {
          systemInstruction: "You are an expert support agent for ZeroLag Games. Help users with game recommendations, order status (simulated), and general platform info.",
        }
      });

      const result = await chat.sendMessage({ message: input });
      const botMsg: Message = { id: (Date.now() + 1).toString(), role: 'bot', content: result.text };
      ...
      setTimeout(() => {
        setMessages(prev => [...prev, { 
          ...
          role: 'bot', 
          content: 'I have logged your request. Our automated sweep system has recorded your active sessions. Please let us know if there is anything else!' 
        }]);
      }, 1000);
    } finally {
      setIsLoading(false);
