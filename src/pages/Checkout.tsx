...
import { collection, serverTimestamp, doc, getDoc, onSnapshot } from 'firebase/firestore';
import { apiService } from '../lib/apiService';
import { uploadMedia } from '../lib/storageHelper';
import { cleanFirestoreData, safeAddDoc, safeUpdateDoc } from '../lib/firestoreUtils';
import { resolveVariantStock, formatItemVersion, applyInventoryDecrement } from '../lib/productNormalizer';
...
export default function Checkout() {
  ...
  const getVersionLabel = (slotTypeOrItem: any) => {
    if (typeof slotTypeOrItem === 'object' && slotTypeOrItem !== null) {
      return formatItemVersion(slotTypeOrItem);
    }
    return formatItemVersion({ selectedSlotType: slotTypeOrItem });
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      // 1. Validate available stock against live DB before settling order & prepare decrements
      const productUpdates: { ref: any; updatedData: any; prodId: string }[] = [];
      ...
      for (const item of cart) {
        ...
        // Determine stock count using universal type-aware inventory resolution
        const { stock: available, label: variantLabel } = resolveVariantStock(prodData, item);

        if (available <= 0) {
          showToast(`Error: "${item.name}" (${variantLabel}) is completely out of stock.`);
          isSubmittingRef.current = false;
          setLoading(false);
          return;
        }
        ...
        if (reqQty > available) {
          showToast(`Error: Only ${available} units of "${item.name}" (${variantLabel}) are available in stock.`);
          isSubmittingRef.current = false;
          setLoading(false);
          return;
        }

        // Calculate decremented product inventory
        const updatedData = applyInventoryDecrement(prodData, item, reqQty);
        productUpdates.push({ ref: prodRef, updatedData, prodId });
      }

      // Primary cart item details for order metadata
      const primaryItem = cart[0];
      const productNames = cart.map(i => i.name).join(', ');
      const selectedVersions = cart.map(i => formatItemVersion(i)).join(', ');
...
========================================`;
      ...
      // 2. Decrement inventory for each ordered product in Firestore
      for (const update of productUpdates) {
        try {
          await safeUpdateDoc(update.ref, update.updatedData);
        } catch (stockErr) {
          console.warn(`Failed to decrement inventory for product ${update.prodId}:`, stockErr);
        }
      }

      // 3. Write order record directly to Firestore orders collection
      await safeAddDoc(collection(db, 'orders'), {
        ...cleanFirestoreData(orderData),
        createdAt: serverTimestamp() // Set Firestore accurate stamp
      });

      // 4. Lock placed order specifics in state before clearing cart
      setPlacedOrder({
        orderId,
        productNames,
        selectedVersions,
      ...
      // 5. CRITICAL IMMEDIATE UI TRANSITION:
      // Transition immediately to WhatsApp confirmation step and clear cart
      setStep(5);
      clearCart();
      showToast('Order saved! Please confirm via WhatsApp.');

      // 6. NON-BLOCKING BACKGROUND PROCESSING:
      // Handle proxy logs, notifications, and AI analysis asynchronously without delaying UI
      ...
      (async () => {
        try {
          ...
          console.warn('Background backend order proxy bypassed:', backendErr);
        }
        ...
        try {
          ...
          console.warn('Background notification dispatch bypassed:', notifyErr);
        }

        try {
          await apiService.getAIInsights(cartSnapshot);
        } catch (aiErr) {
          console.warn('Background AI insights processor bypassed:', aiErr);
        }
      })();
      ...
      showToast('Failed to save order. Please check your connection and try again.');
    } finally {
      setLoading(false);
      isSubmittingRef.current = false;
    }
                      ...
                      {cart.map((item, idx) => (
                        <div key={item.id || idx} className="col-span-full border-b border-white/5 pb-3 last:border-b-0 last:pb-0 space-y-1">
                          ...
                          <div className="flex justify-between">
                            <span>Version:</span>
                            <span className="text-gray-200">{getVersionLabel(item)}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Quantity:</span>
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="space-y-0.5 min-w-0">
                        ...
                        <div className="text-[8px] font-mono text-[#00F0FF] uppercase tracking-wider">
                          Qty: {item.quantity || 1} &nbsp;|&nbsp; {getVersionLabel(item)}
                        </div>
                      </div>
                    </div>
