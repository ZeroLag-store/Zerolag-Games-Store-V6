...
export default function AIRecommender() {
  ...
  const handleSuggest = async (e: React.FormEvent) => {
    ...
    try {
      if (!ai) {
        throw new Error("Gemini API key is missing. Neural insights unavailable.");
      }
      const response = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: `I want to find a game to play. My mood/preference is: "${input}". 
        Suggest 3 games that would be a perfect match. 
        Return the result as a JSON array of objects with 'name', 'reason', and 'matchScore' (0-100).`,
        config: {
