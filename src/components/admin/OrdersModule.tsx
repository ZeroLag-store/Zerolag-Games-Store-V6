...
import { 
  ...
  Image as ImageIcon, HelpCircle, XCircle, Truck, FileText, Mail, Send, Check
} from 'lucide-react';
...
export default function OrdersModule({
  ...
  const handleSendOrderEmail = async (ord: any) => {
    const orderId = ord.orderId || ord.orderNumber || ord.id;
    const targetEmail = ord.customerEmail || ord.userEmail;
    if (!targetEmail) {
      showToast('No customer email provided for this order');
      return;
    }
    ...
    try {
      ...
      await apiService.sendOrderNotification({
        ...
        customerPhone: ord.customerPhone || ord.phoneNumber,
        customerName: ord.customerName,
        total: ord.finalPrice || ord.price || ord.total || 0,
        status: ord.status || 'Verified',
        credentials: ord.credentials || ord.accountEmail || '',
        items: [{
          name: ord.productName || 'ZeroLag Digital Product',
          version: ord.selectedVersion || ord.slotType || 'Standard',
          price: ord.finalPrice || ord.price || ord.total || 0
        }]
      });
      showToast(`Order confirmation & details dispatched to ${targetEmail}`);
      onLogActivity('Email Sent', 'Order Management', `Dispatched order receipt to ${targetEmail} for #${orderId}`);
    } catch (err: any) {
      console.error('Failed to send order email:', err);
      showToast('Notification dispatch logged (Service ready)');
    } finally {
  ...
  // Filter ERP orders listing (deterministic timestamp descending)
  const filteredErpOrders = erpOrders
    .filter(o => 
  ...
  // Filter Web Checkout orders listing (deterministic timestamp descending)
  const filteredWebOrders = webCheckoutOrders
    .filter(o => {
      const oAny = o as any;
      ...
      {activeSubTab === 'list' && (
        <div className="space-y-4">
          ...
          <div className="p-4 sm:p-6 bg-[#151619] rounded-[2rem] border border-white/5 overflow-hidden">
            ...
            <div className="hidden md:block w-full overflow-x-auto rounded-2xl border border-white/5 bg-[#0D0E12]">
              <table className="w-full text-left border-collapse min-w-[1150px] table-auto">
                <thead>
                  <tr className="border-b border-white/5 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 bg-white/[0.02]">
                    <th className="py-4 pl-4 pr-3 min-w-[130px] whitespace-nowrap">Order Code</th>
                    <th className="py-4 px-3 min-w-[120px] whitespace-nowrap">Date</th>
                    <th className="py-4 px-3 min-w-[170px]">CRM Customer</th>
                    <th className="py-4 px-3 min-w-[210px]">Target Credentials</th>
                    <th className="py-4 px-3 min-w-[120px] whitespace-nowrap">Amount EGP</th>
                    <th className="py-4 px-3 min-w-[140px] whitespace-nowrap">Profit Status</th>
                    <th className="py-4 pl-3 pr-4 min-w-[200px] text-right whitespace-nowrap">Ledger Action</th>
                  </tr>
                </thead>
                  ...
                  {filteredErpOrders.length > 0 ? (
                    filteredErpOrders.map((ord) => (
                      <tr key={ord.id} className="hover:bg-white/[0.02]/70 transition-all">
                        <td className="py-5 pl-4 pr-3 font-mono font-black text-white whitespace-nowrap">
                          <span className="bg-white/5 border border-white/10 px-2.5 py-1 select-all rounded font-mono text-xs">
                            {ord.orderNumber}
                          </span>
                        </td>
                        <td className="py-5 px-3 text-gray-400 font-mono text-xs whitespace-nowrap">
                        ...
                        <td className="py-5 px-3 min-w-[170px]">
                          <div className="font-extrabold text-white text-xs sm:text-sm uppercase tracking-wide whitespace-normal">
                            {ord.customerName}
                          </div>
                          {ord.customerPhone && (
                        ...
                        <td className="py-5 px-3 min-w-[210px]">
                          <div className="font-mono text-gray-200 text-xs break-all select-all" title={ord.accountEmail}>
                            {ord.accountEmail}
                          </div>
                          <div className="text-[10px] text-[#00F0FF] uppercase font-black tracking-wide mt-1 whitespace-nowrap">
                            Slot: {ord.slotType.replace('_', ' ')}
                          </div>
                        </td>
                        <td className="py-5 px-3 min-w-[120px] whitespace-nowrap">
                          <span className="inline-block px-2.5 py-1 bg-black/40 border border-white/5 rounded-lg font-mono font-black text-white text-xs whitespace-nowrap">
                            {formatPrice(ord.price)}
                          </span>
                        </td>
                        <td className="py-5 px-3 min-w-[140px] whitespace-nowrap">
                          <select 
                            value={ord.status}
                            onChange={e => onUpdateOrderStatus(ord.id, e.target.value as OrderStatus)}
                            className={`py-1.5 px-2.5 rounded-lg text-[9px] font-black uppercase tracking-wider bg-[#0B0B0F] outline-none border cursor-pointer whitespace-nowrap ${
                              ord.status === 'Completed' ? 'bg-green-500/10 text-green-400 border-green-500/25' : 
                              ord.status === 'Cancelled' ? 'bg-red-500/10 text-red-500 border-red-500/25' : 'bg-yellow-500/10 text-yellow-500 border-yellow-500/25'
                            }`}
                          >
                            <option className="bg-[#151619] text-yellow-500" value="Pending">Pending</option>
                            <option className="bg-[#151619] text-green-400" value="Completed">Completed</option>
                            <option className="bg-[#151619] text-red-500" value="Cancelled">Cancelled</option>
                            <option className="bg-[#151619] text-orange-400" value="Refunded">Refunded</option>
                          </select>
                        </td>
                        <td className="py-5 pl-3 pr-4 min-w-[200px] text-right whitespace-nowrap">
                          <div className="flex items-center justify-end gap-2 shrink-0 flex-nowrap">
                            <button
                              ...
                              className="whitespace-nowrap shrink-0 px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-white rounded-lg text-[10px] font-black uppercase tracking-wider transition-all"
                            >
                              Edit
                            </button>
                            {userRole !== 'EMPLOYEE' && onDeleteOrder && (
                              <button
                                ...
                                className="whitespace-nowrap shrink-0 p-2 bg-red-400/10 hover:bg-red-500 text-red-500 hover:text-black border border-red-500/20 hover:border-transparent rounded-lg transition-all"
                                title="Delete Order"
                              >
                                <Trash2 size={13} />
                              </button>
      ...
      {activeSubTab === 'web-orders' && (
        <div className="space-y-4">
          ...
          <div className="p-4 sm:p-6 bg-[#151619] rounded-[2rem] border border-white/5 overflow-hidden">
            ...
            <div className="hidden md:block w-full overflow-x-auto rounded-2xl border border-white/5 bg-[#0D0E12]">
              <table className="w-full text-left border-collapse min-w-[1360px] table-auto">
                <thead>
                  <tr className="border-b border-white/5 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 bg-white/[0.02]">
                    <th className="py-4 pl-4 pr-3 min-w-[140px] whitespace-nowrap">Order ID</th>
                    <th className="py-4 px-3 min-w-[210px] whitespace-nowrap">Date / Client Details</th>
                    <th className="py-4 px-3 min-w-[240px] whitespace-nowrap">Game & Version License</th>
                    <th className="py-4 px-3 min-w-[140px] whitespace-nowrap">Amount Due & Gateway</th>
                    <th className="py-4 px-3 min-w-[150px] whitespace-nowrap">Upload Screenshot Proof</th>
                    <th className="py-4 px-3 min-w-[190px] whitespace-nowrap">State Status</th>
                    <th className="py-4 pl-3 pr-4 min-w-[340px] text-right whitespace-nowrap">Admin Verification Handlers</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5 text-sm">
                  {filteredWebOrders.length > 0 ? (
                    filteredWebOrders.map((ord: any) => (
                      <tr key={ord.id} className="hover:bg-white/[0.02]/70 transition-all">
                        ...
                        {/* Order ID */}
                        <td className="py-5 pl-4 pr-3 whitespace-nowrap">
                          <span className="font-mono font-black text-white px-2.5 py-1 bg-white/5 border border-white/10 rounded select-all font-mono text-xs whitespace-nowrap">
                        ...
                        <td className="py-5 px-3 min-w-[210px]">
                          ...
                          <div className="text-[10px] text-gray-400 font-mono mt-0.5 whitespace-normal break-all">
                            {ord.customerEmail}
                          </div>
                          {ord.customerPhone && (
                            <div className="text-[10px] text-[#00F0FF] font-mono mt-0.5 whitespace-nowrap">
                              📞 {ord.customerPhone}
                            </div>
                          )}
                          <div className="text-[10px] text-gray-500 mt-1 font-mono font-semibold whitespace-nowrap">
                        ...
                        {/* Game and Version */}
                        <td className="py-5 px-3 min-w-[240px]">
                          <div className="font-bold text-white text-xs sm:text-sm leading-snug uppercase whitespace-normal" title={ord.productName}>
                            {ord.productName || 'Direct Package License'}
                          </div>
                          <div className="text-[10px] text-[#00F0FF] uppercase font-black tracking-wide mt-1.5 flex items-center gap-1.5 whitespace-nowrap">
                            <span className="w-1.5 h-1.5 bg-[#00F0FF] rounded-full shrink-0"></span>
                            <span>Version: {ord.selectedVersion || 'Secondary'}</span>
                            {ord.quantity && <span className="text-gray-500 font-mono">({ord.quantity}x)</span>}
                          </div>
                        </td>

                        {/* Amount and Method */}
                        <td className="py-5 px-3 min-w-[140px] font-mono whitespace-nowrap">
                          <div className="inline-block px-2.5 py-1 bg-black/40 border border-white/5 rounded-lg text-xs font-black text-white whitespace-nowrap">
                            {formatPrice(ord.finalPrice || ord.price || ord.total || 0)}
                          </div>
                          <div className="text-[10px] text-[#25D366] font-bold uppercase tracking-widest mt-1.5 whitespace-nowrap">
                            {ord.paymentMethod || 'InstaPay'}
                          </div>
                        </td>

                        {/* Screenshot Proof */}
                        <td className="py-5 px-3 min-w-[150px] whitespace-nowrap">
                          {ord.paymentProofUrl ? (
                            <div className="flex items-center gap-2">
                              ...
                              <a 
                                ...
                                className="group flex items-center gap-2 p-1.5 px-3 bg-[#00F0FF]/10 hover:bg-[#00F0FF] border border-[#00F0FF]/20 hover:border-transparent rounded-xl text-[#00F0FF] hover:text-black font-black transition-all text-[10px] uppercase tracking-wide whitespace-nowrap shrink-0"
                              >
                                <img 
                                  src={ord.paymentProofUrl} 
                                  alt="Screenshot Proof" 
                                  className="w-5 h-5 object-cover rounded border border-[#00F0FF]/30 group-hover:border-transparent shrink-0"
                                />
                            ...
                            <span className="text-[9px] font-mono font-bold text-yellow-500 border border-yellow-500/20 px-2.5 py-1.5 rounded-lg bg-yellow-500/5 flex items-center gap-1 w-fit whitespace-nowrap">
                        ...
                        {/* State Status Badge */}
                        <td className="py-5 px-3 min-w-[190px] whitespace-nowrap">
                          <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border whitespace-nowrap ${
                            ...
                            <span className="whitespace-nowrap">{ord.status || 'Pending Payment Verification'}</span>
                          </span>
                        </td>

                        {/* Handlers (Mark Paid, Mark Delivered, Cancel Order) */}
                        <td className="py-5 pl-3 pr-4 min-w-[340px] text-right whitespace-nowrap">
                          <div className="flex items-center justify-end gap-2 shrink-0 flex-nowrap">
                            ...
                            {ord.status !== 'Paid' && ord.status !== 'Delivered' && (
                              <button
                                ...
                                className="whitespace-nowrap shrink-0 px-3 py-1.5 bg-emerald-500 text-black hover:bg-emerald-400 rounded-lg font-black text-[10px] uppercase tracking-wider transition-colors shadow-sm"
                            ...
                            {ord.status !== 'Delivered' && (
                              <button
                                ...
                                className="whitespace-nowrap shrink-0 px-3 py-1.5 bg-cyan-500 text-black hover:bg-cyan-400 rounded-lg font-black text-[10px] uppercase tracking-wider transition-colors shadow-sm"
                            ...
                            {ord.status !== 'Cancelled' && (
                              <button
                                ...
                                className="whitespace-nowrap shrink-0 p-2 hover:bg-red-500/10 text-gray-400 hover:text-red-400 border border-transparent hover:border-red-500/25 rounded-lg transition-all"
                            ...
                            <button
                              type="button"
                              disabled={sendingEmailOrderId === (ord.orderId || ord.orderNumber || ord.id)}
                              onClick={() => handleSendOrderEmail(ord)}
                              className="whitespace-nowrap shrink-0 px-2.5 py-1.5 bg-indigo-500/10 hover:bg-indigo-500 text-indigo-400 hover:text-white border border-indigo-500/20 hover:border-transparent rounded-lg font-black text-[10px] uppercase tracking-wider transition-all flex items-center gap-1"
                              title="Email Confirmation & Delivery Details"
                            >
                              {sendingEmailOrderId === (ord.orderId || ord.orderNumber || ord.id) ? (
                            ...
                            <button
                              ...
                              className="whitespace-nowrap shrink-0 px-3 py-1.5 bg-[#00F0FF]/15 border border-[#00F0FF]/25 text-[#00F0FF] hover:bg-[#00F0FF] hover:text-black rounded-lg font-black text-[10px] uppercase tracking-wider transition-all"
                            >
                              Review & Receipt
                            </button>
                            ...
                            {userRole !== 'EMPLOYEE' && onDeleteOrder && (
                              <button
                                ...
                                className="whitespace-nowrap shrink-0 p-2 bg-red-400/10 hover:bg-red-500 text-red-400 hover:text-black border border-red-500/20 hover:border-transparent rounded-lg transition-all"
                                title="Delete Order"
                              >
                                <Trash2 size={14} />
                              </button>
...
========================================`}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-white/5">
              <div>
                {(editingOrder.customerEmail || editingOrder.userEmail) && (
                  <button
                    type="button"
                    disabled={sendingEmailOrderId === (editingOrder.orderId || editingOrder.orderNumber || editingOrder.id)}
                    onClick={() => handleSendOrderEmail(editingOrder)}
                    className="px-4 py-2.5 bg-indigo-500/10 hover:bg-indigo-500 text-indigo-400 hover:text-white border border-indigo-500/25 hover:border-transparent rounded-xl text-[9px] font-black uppercase tracking-wider transition-all flex items-center gap-1.5"
                  >
                    {sendingEmailOrderId === (editingOrder.orderId || editingOrder.orderNumber || editingOrder.id) ? (
                      <RefreshCcw size={13} className="animate-spin" />
                    ) : (
                      <Mail size={13} />
                    )}
                    <span>Dispatch Email to {editingOrder.customerEmail || editingOrder.userEmail}</span>
                  </button>
                )}
              </div>
