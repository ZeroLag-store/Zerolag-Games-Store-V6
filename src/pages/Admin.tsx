...
import { safeAddDoc, safeUpdateDoc, safeSetDoc, cleanFirestoreData } from '../lib/firestoreUtils';
...
export default function Admin() {
  ...
  const handleAddAccount = async (acc: Partial<DigitalAccount>) => {
    try {
      ...
      await safeAddDoc(collection(db, 'accounts'), payload);
    } catch (err) {
  ...
  const handleEditAccount = async (accountId: string, acc: Partial<DigitalAccount>) => {
    try {
      ...
      await safeUpdateDoc(doc(db, 'accounts', accountId), payload);
    } catch (err) {
  ...
  const handleBulkUpdateStatus = async (accountIds: string[], slotType: string, status: string) => {
    try {
      for (const id of accountIds) {
        ...
        await safeUpdateDoc(doc(db, 'accounts', id), {
          slots: cleanFirestoreData(updatedSlots),
          updatedAt: new Date().toISOString()
        });
      }
    } catch (err) {
  ...
  const handleAddCustomer = async (cust: Partial<Customer>) => {
    try {
      ...
      await safeAddDoc(collection(db, 'customers'), payload);
    } catch (err) {
  ...
  const handleEditCustomer = async (customerId: string, cust: Partial<Customer>) => {
    try {
      await safeUpdateDoc(doc(db, 'customers', customerId), cleanFirestoreData(cust));
    } catch (err) {
  ...
  const handleAddOrder = async (ord: Partial<Order>) => {
    try {
      const ordPayload = cleanFirestoreData({
        ...ord,
        createdAt: new Date().toISOString()
      });
      const docRef = await safeAddDoc(collection(db, 'orders'), ordPayload);
      ...
      if (acc) {
        ...
        await safeUpdateDoc(doc(db, 'accounts', ord.accountId!), {
          slots: cleanFirestoreData(updatedSlots),
          updatedAt: new Date().toISOString()
        });
      }
      ...
      if (cust) {
        const newSpending = (cust.totalSpending || 0) + (ord.price || 0);
        await safeUpdateDoc(doc(db, 'customers', ord.customerId!), {
      ...
      const warrantyPayload = cleanFirestoreData({
        orderId: docRef.id,
        customerId: ord.customerId || '',
        customerName: ord.customerName || '',
        productName: ord.accountEmail || ord.productName || '',
        slotType: ord.slotType || 'PRIMARY',
        startDate: new Date().toISOString(),
        endDate: oneYearLater.toISOString(),
        status: 'ACTIVE'
      });

      await safeAddDoc(collection(db, 'warranties'), warrantyPayload);
    } catch (err) {
  ...
  const handleUpdateOrderStatus = async (id: string, status: any) => {
    try {
      await safeUpdateDoc(doc(db, 'orders', id), { status });
  ...
  const handleEditOrder = async (id: string, updatedFields: any) => {
    try {
      await safeUpdateDoc(doc(db, 'orders', id), cleanFirestoreData(updatedFields));
  ...
  const handleCreateReceipt = async (receipt: Partial<Receipt>) => {
    try {
      ...
      await safeAddDoc(collection(db, 'receipts'), payload);
    } catch (e) {
  ...
  const handleUpdateWarrantyStatus = async (id: string, status: any) => {
    try {
      await safeUpdateDoc(doc(db, 'warranties', id), { status });
  ...
  const handleUpdateReservationStatus = async (id: string, status: any) => {
    try {
      await safeUpdateDoc(doc(db, 'reservations', id), { status });
  ...
  return (
    <div className="w-full max-w-[1720px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-24 space-y-8 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-9 relative min-w-0 w-full">
          {activeTab === 'dashboard' && (
            <DashboardModule 
              stats={activeStats}
